...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, a.id,b.group_id, password, active, last_login')
						  ->join('users_groups b','a.id=b.user_id')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('a.id', 'desc')
						  ->get($this->tables['users'].' a');
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'group_id'             => $user->group_id, //everyone likes to overwrite id so we'll use user_id
